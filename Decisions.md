...
RESOLVED: portfolio_positions had a UNIQUE(ticker) constraint from before 
the multi-trader migration. Multi-trader requires UNIQUE(trader_id, ticker) 
— two traders can hold the same stock independently. Fixed by dropping the 
single-column constraint and replacing with a composite one.